...
The core library is written in idiomatic Rust without concern for binding to other languages. The Rust API won't always correlate 100% with the binding APIs. This guide is helpful for Rust users, but is primarily written from the perspective of the bindings.
