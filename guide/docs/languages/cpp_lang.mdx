...
is called on a moved class it will throw a `std::logic_error`.
...
from a C++ `std::future` using the blocking `get` method and there is no way to asynchronously chain futures.
