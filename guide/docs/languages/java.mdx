...
The Java bindings are distributed as a JAR targeting Java 8. Native libraries are embedded in the JAR libraries for Windows and Linux in `resources`. The
correct native library will automatically load during a static initialization. These native libraries wrap the underlying C API with a thin layer of
[JNI](https://docs.oracle.com/javase/8/docs/technotes/guides/jni/).

## Maven

Release artifacts are published to Maven central. Add this code to incorporate them into your projects:
...
Java is an object-oriented language that support all the abstract patterns modeled in the code generator. This section describes those mappings.
...
:::warning
Uncaught exceptions thrown in callbacks will terminate the program. Always wrap your code with callback logic using `try/catch` syntax if there's a possibility the callback will throw.
:::
...
used outside the callback, such as being sent to another thread for processing.

### Structs

Native structs are mapped to Java classes. They have public member visibility, while the constructor ensures that all values are initialized properly.
