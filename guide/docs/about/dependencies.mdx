...
To ensure that our binary distributions meet the legal requirements of the OSS licenses of these dependencies, we have developed an automated tool called
`complicense` that performs the following tasks:

* Analyzes each dependency's license against a whitelist. Our CI builds will fail if a dependency is added that is not allowed.
* Uses the Github API is automatically retrieve the license file for each project hosted on Github. Only a few projects don't have the proper metadata and the
...
* Produces a license report document called `licenses.txt` that rolls up all of the dependency and license information and is included in all of our binary
distributions.
...
The tool produces a verbose report called [licenses.txt](https://github.com/stepfunc/dnp3/blob/master/ffi/licenses.txt) that aggregates the following information for each license:
