...
* It is not a general purpose library for implementing DNP3 outstations on microcontrollers. The library supports platforms and architectures supported by Rust
and the [Tokio](https://tokio.rs/) Runtime.
* The library does not implement some uncommon or dangerous DNP3 features. Namely, secure authentication version 2/5, will never be implemented in this library
and we discourage others from using them in production environments. TLS is always a better choice when IP communications is available. Secure authentication
version 6 *may* be implemented in the future.

## Interoperability

DNP3 defines four subset levels for the purpose of interoperability:

* `Level 1` - The *minimum* feature set that a device must implement to be DNP3 compliant. The functionality is quite basic, intended for a small field device.
...
This library is a subset **Level 3** compliant implementation, both master and outstation. It also implements a number of additional protocol features:
