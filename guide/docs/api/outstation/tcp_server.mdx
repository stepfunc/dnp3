...
Bind one or more outstation instances to a TCP server using an `AddressFilter`. Unlike most server protocols, each outstation can only communicate with one client at a time because DNP3 maintains event data for a specific master. To support multiple masters on the same port, preconfigure each master IP address and associate it with a specific outstation instance.
...
Create a `TCPServer` instance. Creating a server doesn't initiate `bind/listen` immediately. Specify endpoints as `<address>:<port>` where address is any valid IPv4 or IPv6 local endpoint:
