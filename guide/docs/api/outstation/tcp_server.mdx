...
The `ConnectionStateListener` interface is only specified for connection oriented transport (e.g. TCP) when adding an outstation to the server. It has a single
method that informs user code when a TCP connection has been established to the outstation. The listener only fires a connected event when an IP address matching
the `AddressFilter` is received. If the outstation is already connected and processing a connection, and another matching connection is received, then the listener
