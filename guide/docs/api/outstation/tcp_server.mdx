...
You can bind one or more outstation instances to a TCP server using `AddressFilter`. Unlike most server protocols, each outstation instance can only
...
one master connection on the same port, you must preconfigure each master IP address and associate it with a specific outstation.
...
Begin by creating a `TCPServer` instance. Keep in mind that creating a server doesn't initiate `bind/listen`. Form your endpoint using `<address>:<port>` to avoid `TCPServer` failure. Addresses may be any specified as any valid IPv4 or IPv6 local endpoint, such as:
...
```c
{{#include ../ffi/bindings/c/outstation_example.c:tcp_server_decl}}
{{#include ../ffi/bindings/c/outstation_example.c:common_error_code}}
...
You can now associate one or more outstations with the `TCPServer`. The `TcpServer::addOutstation` method takes all of the components discussed in previous sections, followed by an `AddressFilter`. This filter determines which masters are allowed to associate with a specific outstation. If the filter overlaps with the filter
of an existing outstation, such as allowing the same address, then `TcpServer::addOutstation` will fail.
...
The examples above use `AddressFilter.any()` to connect to any master IP address:
...
If you've already connected an outstation to a master and another matching IP address tries to connect, the existing connection will close. The outstation will then begin a communication session with the new master.

## ConnectionStateListener

The `ConnectionStateListener` interface is only specified for connection-oriented transport such as TCP when adding an outstation to the server. It has a single
method that informs your code when the outstation establishes a TCP connection. The listener only fires a connected event when it receives an IP address matching
the `AddressFilter`. If the outstation that is already connected and processing a connection receives another matching connection, the listener
will fire a disconnected event before firing a connected event.


## Binding the Server

Once you've added all the outstations associated with the server, you can then bind the server and start accepting connections.
