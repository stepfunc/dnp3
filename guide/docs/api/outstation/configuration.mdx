...
The `EventBufferConfig` struct controls the number of events buffered for each type that the outstation can report. During the outstation initialization, you will pre-allocate space for each type. Once all the space in the buffer for a particular type is used, each new event will discard the oldest event. The outstation will then assert IIN 2.3 (Event Buffer Overflow) per the specification.
