...
`OutstationApplication` provides callbacks for dynamic events and information the outstation requires:
...
The outstation calls `OutstationApplication::getApplicationIin()` for every solicited or unsolicited response, merging your `ApplicationIin` bits with internal IIN bits. You control four bits:
...
* `freezeCountersAll`: Called for freeze operations using the All Points (0x06) qualifier.
* `freezeCountersRange`: Called for freeze operations using 8-bit (0x00) or 16-bit (0x01) range qualifiers.

Use the provided `DatabaseHandle` to copy counters to frozen counters. `FreezeType` specifies the operation:
...
1. If the attribute has not been defined the outstation returns `IIN2.NO_FUNC_CODE_SUPPORT`.
