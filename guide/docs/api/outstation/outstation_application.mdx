...
The `OutstationApplication` provides callbacks for dynamic events and information that the outstation requires from the user application:
...
The outstation will call `OutstationApplication::getApplicationIin()` whenever is performing a solicited or unsolicited response. It will merge
...
`RestartDelay` with a time estimate of how long the restart will take in seconds or milliseconds. This delay returned to the master isn't really of much
practical use to real world masters so don't worry too much about the accuracy of this value.
