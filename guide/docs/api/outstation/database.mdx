...
:::tip
The `UpdateOptions` parameter allows the user to precisely control how a point update is processed. The default constructor for this struct has the following
behavior:
...
Ignoring event creation is useful during startup initialization if you don't want to create an event for the initial value.
:::
