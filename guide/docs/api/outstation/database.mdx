...
All database operations are executed in a transaction. All the operations within a transaction will be applied to the database and the
event buffers at the same time. If unsolicited responses are enabled, the outstation will automatically decides if an unsolicited response
should be sent at the end of the transaction.

Database transactions can occur in 2 distinct ways:
...
2. When a control-carrying ASDU is received (select, operate, etc), the outstation locks the database and passes a reference
to it into each callback anticipating updates from the user.

:::tip
Structure any common update code to operate on the `Database` type and not depend on `Outstation`. This will allow you to easily
share code for the two ways in which a database transaction may occur.
:::
