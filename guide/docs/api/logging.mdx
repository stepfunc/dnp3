...
* `LogLevel` that generates messages 
...
:::note
Protocol decoding is always output at the *Info* log level. If left enabled, it can be extremely lengthy in a production system. When you're debugging a communication issue, try adjusting the application-layer decoding first to gain visibility into the messages exchanged.
:::
