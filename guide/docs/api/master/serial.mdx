...
Unlike the outstation API, the master API opens the serial port lazily after `MasterChannel.enable()` is called. It will try to reopen the port
...
```rust
{{#include ../examples/master/src/main.rs:create_master_serial_channel}}
```
