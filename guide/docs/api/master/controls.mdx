...
The asynchronous operation will return a single `CommandResult` enum that indicates the result. `CommandResult.Success` means the outstation responded with `Success` for all controls in the message. Consult the language-specific documentation for the meaning
of each error code.