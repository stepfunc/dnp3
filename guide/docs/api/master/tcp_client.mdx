...
configurations, and a callback to observe the status of the connection.
...
close the connection. However, you can treat the TCP like a serial port by changing the setting to `LinkErrorMode::Discard`.
...
The `ClientStateListener` interface has a single method to inform the application about the state of connection.