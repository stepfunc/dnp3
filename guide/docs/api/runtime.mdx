...
dnp3_param_error_t err = dnp3_runtime_new(runtime_config, &runtime);
if (err) {
    // technically, runtime creation can fail, but would require OS
    // resource exhaustion to do so.
}
...
:::warning
Applications written in languages that use exceptions for error handling (Java/C#) must ensure that exceptions thrown within callbacks are always caught.
Letting an exception propagate onto a thread-pool thread will result in program termination.
:::
