...
The DNP3 library runs on top of the [Tokio](https://tokio.rs/) runtime which provides a state-of-the-art work stealing scheduler and platform-agnostic
...
after initializing logging and is the last component to shutdown. Shutting down a `Runtime` will stop all of the communication sessions that were created with it.
...
```rust
{{#include ../../../dnp3/examples/master_tokio_main.rs:runtime_init}}
...
```c
{{#include ../../../ffi/bindings/c/main.c:runtime_init}}

// ----- run your program ------

// destroy the runtime before the program exits
{{#include ../../../ffi/bindings/c/main.c:runtime_destroy}}
```
...
```
:::note
Prefer try-with-resources syntax as Runtime implements *java.lang.AutoCloseable*.
:::
...
:::note
Prefer using statements as Runtime implements *System.IDisposable*.
:::
...
:::tip
When using any of the bindings, setting the number of runtime threads to `0` will default to the number of system cores.
:::
