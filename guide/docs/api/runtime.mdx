...
The DNP3 library runs on top of the [Tokio](https://tokio.rs/) runtime, giving you state-of-the-art scheduler and platform-agnostic networking APIs. The OS-specific mechanisms we use are *epoll* for Linux and *IOCP* for Windows. 

Tokio is a modern evolution of libraries like [libuv (C)](https://libuv.org/) and [ASIO (C++)](https://think-async.com/Asio/). It leverages Rust's thread and memory safety to deliver asynchronous programs that are incredibly fast and accurate. This can be extremely helpful since it it is difficult to write accurate asynchronous software in C/C++ due to the need to manually reason object lifetimes in callbacks.
...
:::note
Rust users can share the runtime with other libraries that also use Tokio. The bindings don't currently allow for runtime sharing, but may do so in a future release.
:::
...
:::tip
When using any binding, set the number of runtime threads to `0` to default to the number of system cores. This provides a safe default that will lead to good multi-core utilization.
:::

## Callbacks

The runtime's thread pool invokes callbacks from the library to user code. The result is that blocking during a callback will make an entire thread unavailable for task execution. If all threads in a thread pool are blocked, communication sessions will not execute.
...
For best results, avoid blocking whenever possible in your applications. Instead, applications should defer blocking calls to dedicated worker threads, such as a user-managed thread that writes log messages to file.
...
When you shut down a `Runtime`, it stops all master and outstation communication associated with it. This is typically the last DNP3-related operation your program should perform before exit.
